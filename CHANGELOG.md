...
- **`_meta/last-run.json` recording.** Curator agent writes `curator` block on every successful run (`ran_at`, `by`, `inbox_processed`, `articles_written`, `drafts_routed`, `head_before`, `head_after`, `forced`); linter agent writes `linter` block (`ran_at`, `by`, `findings_count`, `forced`). Both preserve the other side via jq merge. The linter's write is the SOLE explicit exception to the read-only invariant (linter MUST 8 + MUST 9). Atomicity: the run-record commit only fires on a clean run; partial / crashed runs leave no record, so the linter's `[CURATOR-IDLE]` clause naturally fires on the next vault audit.
- **CLI authoritative invariant whitelist (P5).** `bin/kunskap lint` enumerates every path touched between pre- and post-snapshots (commits since `head_before` AND new working-tree porcelain entries) and refuses any path that is not `_meta/last-run.json`. Exit code 2 with a stderr listing of disallowed paths. Closes the gap between the linter's whitelisted single-write and the read-only contract (P4 §3 head-oid + porcelain pair generalized).
