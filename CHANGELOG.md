...
- `hooks/hooks.json` + `session-start.sh` + `session-end.sh` + `_shared.sh` — SessionStart pull, SessionEnd commit-and-push for shared vaults only. **Hard fail-soft contract**: every error path exits 0 with a stderr message, never blocks session start/end.
...
- `learn enable` requires interactive confirmation unless `--yes` or `KUNSKAP_AUTO_CONFIRM=1` (Risk #7 privacy primitive).
- `learn status` cross-checks identity against `roles.toml` primaries and reports primary / not-primary (Risk #5 — identity fragility).
