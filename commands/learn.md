...
- For `enable`: tell the user `/kunskap:learn status` will surface vault + identity + role-check + staleness.
- For `status`: surface the structured output as-is. If it warns about >30d staleness or identity-not-set, repeat the warning at the end so the user sees it.
- For `disable`: confirm the marker is gone with a one-liner.
